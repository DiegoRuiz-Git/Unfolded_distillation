...
px = np.logspace(-7,np.log10(5e-5),10)[px_index]
circuit = AddNoise(circuit, px = px, pz = 1e-3)
...
# Z detector of surface code
Z_detector = [detector_order[i] for i in np.where(stabilizer_type == 'Z')[0] if i != merged_Z_stab]
Z_detector = [detector for row in Z_detector for detector in row]

# add detector of final measurement round
tot_meas = sum(len(sublist) for sublist in detector_order)
Z_detector += [tot_meas + (int(dx/2)+1)*(dz-1) + i for i in range(int(dx/2)*dz)]
Z_detector = np.array(Z_detector)

# detector error model associated to Z stabilizers
Z_dem = stim.DetectorErrorModel()

for instruction in dem:
    detector_index = np.array([detector.val for detector in instruction.targets_copy() if str(detector)[0] == 'D'])
    if np.all(np.isin(detector_index, Z_detector)):
        target_copy = []
        for det in instruction.targets_copy():
            if str(det)[0] == 'D':
                target_copy.append(stim.DemTarget.relative_detector_id(det.val))
        if len(target_copy)>0:
            Z_dem.append("error", instruction.args_copy()[0], target_copy)

# edge of surface code to attach logical operator
Z_obs_attached = [detector_order[i] for i in np.where(stabilizer_type == 'Z')[0] if i != merged_Z_stab]
Z_obs_attached = [detector for row_index in range(0,len(Z_obs_attached),2) for detector in Z_obs_attached[row_index]]

tot_meas = sum(len(sublist) for sublist in detector_order)
Z_obs_attached += [tot_meas + (int(dx/2)+1)*(dz-1) + i for i in range(0,int(dx/2)*dz,2)]
Z_obs_attached = np.array(Z_obs_attached)

# dem of complementery gap
Z_dem_complementary = stim.DetectorErrorModel()

for instruction in Z_dem:
    target_copy = []
    for det in instruction.targets_copy():
        if str(det)[0] == 'D':
            new_det_index = np.where(Z_detector==det.val)[0][0]
            target_copy.append(stim.DemTarget.relative_detector_id(new_det_index))
        if len(instruction.targets_copy()) == 1 and det.val in Z_obs_attached:
            target_copy.append(stim.DemTarget.relative_detector_id(len(Z_detector)))
    Z_dem_complementary.append("error", instruction.args_copy()[0], target_copy)

# compute complementary gap
Z_detection_events = np.hstack((detection_events[:,Z_detector],observables[:,4].reshape(-1, 1)))
matching = pymatching.Matching.from_detector_error_model(Z_dem_complementary)
weights = matching.decode_batch(Z_detection_events,return_weights=True)[1]

Z_detection_events_complementary = np.hstack((detection_events[:,Z_detector],~observables[:,4].reshape(-1, 1)))
weights_complementary = matching.decode_batch(Z_detection_events_complementary,return_weights=True)[1]

complementary_gap = np.abs(weights - weights_complementary)

# -------------------------------
# calculate results
# -------------------------------

# complementary gap threshold for postselection
threshold = np.percentile(complementary_gap, 8)
gap = (np.abs(complementary_gap) > threshold - 1)
...
result_dict['preselected&accepted'] = [int(np.sum(accepted))]
result_dict['preselected&fail'] = [int(np.sum(fail))]
result_dict['preselected&gap'] = [int(np.sum(gap))]
...
result_dict['preselected&accepted&gap'] = [int(np.sum(accepted&gap))]
result_dict['preselected&noflag&fail'] = [int(np.sum(noflag&fail))]
result_dict['preselected&noflag&gap'] = [int(np.sum(noflag&gap))]
result_dict['preselected&fail&gap'] = [int(np.sum(gap&fail))]

result_dict['preselected&accepted&noflag&fail'] = [int(np.sum(accepted&noflag&fail))]
result_dict['preselected&gap&noflag&fail'] = [int(np.sum(gap&noflag&fail))]
result_dict['preselected&accepted&gap&fail'] = [int(np.sum(accepted&gap&fail))]
result_dict['preselected&accepted&noflag&gap'] = [int(np.sum(accepted&noflag&gap))]

result_dict['preselected&accepted&noflag&gap&fail'] = [int(np.sum(accepted&noflag&gap&fail))]
...
for preselected_rounds in range(1,rounds):
    ...
    result_dict['preselected&accepted'].append(int(np.sum(preselect&accepted)))
    result_dict['preselected&fail'].append(int(np.sum(preselect&fail)))
    result_dict['preselected&gap'].append(int(np.sum(preselect&gap)))
    ...
    result_dict['preselected&accepted&gap'].append(int(np.sum(preselect&accepted&gap)))
    result_dict['preselected&noflag&fail'].append(int(np.sum(preselect&noflag&fail)))
    result_dict['preselected&noflag&gap'].append(int(np.sum(preselect&noflag&gap)))
    result_dict['preselected&fail&gap'].append(int(np.sum(preselect&gap&fail)))

    result_dict['preselected&accepted&noflag&fail'].append(int(np.sum(preselect&accepted&noflag&fail)))
    result_dict['preselected&gap&noflag&fail'].append(int(np.sum(preselect&gap&noflag&fail)))
    result_dict['preselected&accepted&gap&fail'].append(int(np.sum(preselect&accepted&gap&fail)))
    result_dict['preselected&accepted&noflag&gap'].append(int(np.sum(preselect&accepted&noflag&gap)))

    result_dict['preselected&accepted&noflag&gap&fail'].append(int(np.sum(preselect&accepted&noflag&gap&fail)))

# -------------------------------
# save
# -------------------------------

output_path = f"Results/result_{px_index}_{job_number}.json"
