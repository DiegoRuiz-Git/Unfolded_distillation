...
dict_load = json.load(open('../CNOT order/UnfoldedCode_d=9.json', 'r'))
...
result_dict['preselected&accepted&fail'] = [int(np.sum(fail&accepted))]

# preselection
for preselected_rounds in range(1,rounds):
    last_round_index = np.array(detector_order[:-len(out_qubits)+1])[:,-preselected_rounds:].flatten()
